...
7. **Do not pad word count past the 2,100 hard floor.** Per Zia (2026-09-16): if a chapter is already at or above 2,100 words, leave it alone — do not add filler just to reach the higher 2,300-2,500 target range. Only expand a chapter that is genuinely under 2,100. **Clarified further by Zia (2026-09-15/16, this pass): a chapter within roughly 70 words of the 2,100 floor (e.g. 2,031-2,099) does NOT need padding either — flag it as "close to floor" in the log and move on. Only genuinely short chapters (well under 2,000) need real content added.**
8. Log format:
...
> ⚠️ **DO NOT TRUST THIS FILE FOR STATUS.** This HANDOFF.md is a reference/notes file only — it goes stale relative to the live repo. Any session picking up this project must check the actual current state directly on GitHub (the real files under `chapters/`, their real word counts, and `beat_map.md`) before trusting any claim made here, including anything below marked "COMPLETE," any word-count table, or any chapter list. Read the live files first, always. **NOTE (2026-09-16): the automated proofreading workflow (`.github/workflows/proofread.yml`) has been PAUSED (push + schedule triggers commented out) per Zia's request, to stop it racing against manual HANDOFF.md edits during live sessions. It can still be run manually via workflow_dispatch. Re-enable by uncommenting the triggers in that file — note that editing files under `.github/workflows/` requires the GitHub connector to have "workflow" OAuth scope, which it did not have as of this note, so a session may need to ask Zia to do it manually via the Actions tab, or re-authorize the connector with that scope first.**
...
- Wren is 20, a Finder apprentice. Ch.12, ch.26, ch.36 all cost her real physical/magical strain; ch.36 nearly cost her the most when the redirection ritual almost concentrated entirely onto her before the group forced an even redistribution. **Ch.12 specifically: Wren performs an unsupervised full read on Ruth's late husband's pocket watch against her own safety guidance, gets a fragmentary vision (a woman's hands, faceless, working something small and fast) and a real physical cost (nosebleed, near-collapse) — this is her first real "full read" price paid, escalating from the shallow reads established earlier.**
...
- Priya's brother Dev joined the investigation ch.24 onward; ch.36-38 show him and Priya's sibling relationship as steady and close, with a hint of something more between Dev and Priya left open but undeveloped by book's end (worth revisiting in Book 3 if wanted). **Ch.8 establishes Dev, serving community-service hours at the substation, drops the "old-timer folk story" seed line to Mara/Wren/Caleb/Priya on the way out — this is the first appearance of the "pie lady never aging" thread that later leads to Walt Pruitt. Ch.9 develops this directly: Priya follows up privately with Dev, learns the fuller Walt Pruitt warning ("never ask Denise for a favor you're not willing to pay back double"), and the two reach a real reconciliation beat about trust rebuilt since his sentence — Dev agrees to take her to Walt as "just family," not a formal interview. Ch.19 adds a direct, previously-unstated detail consistent with this: Priya herself was the one who "finally had enough on my own brother to bring him in" — i.e., Priya reported Dev, which is what led to his sentence. Confirms and sharpens the existing arc rather than contradicting it.**
- Yusuf, permanent council chair, authorized the formal inquiry (ch.25) and took part in the full climax. **Ch.10-11 establish his personal stake explicitly: Denise sat with his dying mother through her final weeks, which is why he stalls before authorizing even an informal look — resolved ch.11 when he grants quiet, informal authorization (not a full inquiry) after personally sitting with Denise for an hour and finding nothing conclusively wrong, an honest half-measure rather than a clean yes.**
...
- Denise's real birth year: 1938 (Odette, ch.21). She is alive, aging naturally, living alone with a quiet rotation of valley check-ins as of ch.41, visited twice by Caleb. **Denise has visible arthritis in her left hand as of the present-day story (established ch.12, Priya's observation) — a small physical detail worth tracking if any later chapter needs to confirm or rule out whether a hands-only glimpse of her belongs to the present or to an old memory.**
- **Ambrose Whitlock** — named in Adelaide's letter as the one who taught her the bargain, predating even her. Odette's ch.21 family stories suggest the practice predates the town itself, one teacher per generation. DELIBERATELY UNRESOLVED. This is the reserved Book 3 hook (ch.41). **Ch.11 adds a small, deliberate piece of foreshadowing consistent with this: Yusuf's mother and Denise, as girls, once stole peaches from "the Whitlock orchard" and were caught by "old man Whitlock himself" — plants the Whitlock name naturally, decades before Adelaide's letter surfaces it directly.**
...
- **Wren's age (reinforced 2026-09-16):** confirmed 20 (ch.2's "Twenty years old now"). Two separate later drift errors found and fixed: ch.12 had Priya call her "a nineteen-year-old" (fixed to twenty), and ch.18 had Wren say "I'm nineteen" about herself, twice in the same exchange (fixed to twenty). This is now the second and third time this exact drift has appeared — treat Wren's age as a specifically high-risk recurring error and double-check it in every remaining chapter where her age comes up, not just a one-off fix.
...
- 2026-09-16 correction from Zia: continuity/voice review MUST proceed in strict chapter-number order (see procedure rule 1a above), not jump to whichever chapter the proofread report flags as worst. This caught a real cross-chapter continuity bug (the six-weeks/four-weeks wedding timeline error spanning ch.1 and ch.4) that a non-sequential, report-driven pass would likely have missed since ch.4 wasn't separately flagged by the mechanical proofreader (that tool doesn't check cross-chapter date consistency). It also caught the Wren-age drift (ch.12, ch.18) the same way.
- 2026-09-16 lesson from this pass: when fixing a "the kind of ___" or similar tell phrase, re-scan the FULL fixed chapter for the exact phrase again after pushing — a rewritten sentence can accidentally reintroduce the same banned construction nearby (happened in ch.8, where a word-count-expansion edit was fine but one of the three original "kind of" instances was mis-fixed into a phrasing that still contained "kind of," caught only on a follow-up grep). **A second, distinct version of this lesson: a word-count expansion pass can accidentally INTRODUCE a fresh instance of a different banned word (happened in ch.13 — an added closing paragraph used "particular stillness," immediately caught and fixed by the same post-edit re-scan habit). Always re-scan for ALL banned terms after ANY edit, not just the one you were originally fixing.** A third instance happened in ch.14 this pass: an initial fix pass caught 2 of 3 "kind of" instances and missed a 3rd ("the same kind of work"), caught only on a deliberate full re-scan after the first push — reinforces that a single scan pass is not reliably exhaustive.
- 2026-09-15/16 clarification from Zia (this pass): don't be pedantic about the word-count floor. A chapter at 2,031 or 2,088 is functionally fine — flag it as "close to floor" in the log and move on rather than padding for the sake of a round number. Work in batches of 3 chapters, push GitHub updates after each batch (not after every single chapter), to manage session length efficiently.
- 2026-09-16, Zia: only Book 2 is in scope right now; Book 3 review/fixes are explicitly paused regardless of what Book 3's own PROOFREAD_REPORT.md shows.
- 2026-09-16, Zia: the automated `proofread.yml` GitHub Actions workflow has been paused (push + schedule triggers disabled) to stop it from racing against manual sessions' HANDOFF.md edits and causing SHA conflicts mid-session. See the DO-NOT-TRUST-THIS-FILE note near the top for how to re-enable it.
...
- **ch.9 — issues found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-15**
  - **AI-tell (FIXED):** removed 2 instances of "the kind of" ("the kind of superstition every small town's got" → "ordinary superstition, the sort every small town collects"; "exactly the kind of attention" → "exactly the attention").
  - Word count 2,088 — close to the 2,100 floor (12 words under). Per Zia's clarified rule this pass, left as-is rather than padded.
  - No em dashes, no "particular"/"the specific." Voice and canon consistent: Dev's supply-closet characterization (precise, self-grading habits from his rebuilding-trust arc), the Walt Pruitt seed line developed directly from ch.8, and Priya/Dev's reconciliation beat all check out and strengthen rather than alter established canon (see updated canon note above).
- **ch.10 — issues found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-15**
  - **AI-tell (FIXED):** removed 4 instances of "the kind of" (rules-for-this-thing, cost-you-can't-write-down, destroys-a-good-woman's-life, and the-emergency lines, all reworded to drop the construction without changing meaning).
  - Word count 2,271 — comfortably above floor, no padding needed.
  - No em dashes, no "particular"/"the specific." Voice and canon consistent: Yusuf's personal stake (his mother's final weeks, Denise's role in them) is new but doesn't contradict anything established — strengthens his ch.25 formal-inquiry arc by giving his hesitation real weight. Added to established canon above. Mara/Caleb/Wren/Priya's dynamic in the confrontation scene all read consistent with their established voices.
- **ch.11 — issues found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-15**
  - **AI-tell (FIXED):** removed 2 instances of "the kind of" (vigilante-justice line, patient-boring-work line).
  - Word count 2,031 — close to the 2,100 floor (69 words under). Per Zia's clarified rule this pass, flagged and left as-is rather than padded.
...
- **ch.12 — issue found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - **Continuity/canon error (FIXED):** Priya's dialogue referred to Wren as "a nineteen-year-old," directly contradicting established canon (Wren is 20, confirmed explicitly in ch.2's "Twenty years old now" and the Established Canon section above). Corrected to "twenty-year-old."
  - Word count 2,159 — already above the 2,100 floor, left as-is per the no-padding rule.
  - No AI-tell instances (particular/the specific/the kind of all zero). No em dashes. Voice and canon otherwise consistent: this is Wren's first full unsupervised read (matches the escalation pattern from her established shallow-reads-only caution), the "hands, no face" vision content is consistent with what she reports again in ch.13, and Denise's established arthritis detail (added to canon above) gives Priya's analysis real evidentiary weight rather than speculation.
- **ch.13 — issue found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - **Word count (FIXED):** 1,995 words, genuinely under the 2,100 floor. Expanded to 2,195 via an interiority-only closing beat (Mara's thoughts before falling asleep) — no new plot, no new facts.
  - **AI-tell (self-caught during the same edit):** the added closing paragraph initially included "that particular stillness," which would have reintroduced the single most-banned word in the whole manuscript. Caught and fixed to "that exact stillness" before pushing, via the routine post-edit re-scan habit noted in standing rules above.
...
- **ch.14 — issues found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - **AI-tell (FIXED, two pushes):** found and fixed 2 instances of "the kind of" on first pass ("exactly that kind of purpose" → "exactly that purpose", "the kind of thing every farm..." → "Something every farm..."). A follow-up full re-scan after push caught a 3rd missed instance ("the same kind of work she'd have done for a bridge inspection") — fixed in a second push. Reinforces the standing-rules lesson: a single scan is not reliably exhaustive.
  - Word count comfortably above 2,100 floor, no padding needed.
  - No em dashes. Continuity: the well/cellar incident split (4/2/1) matches exactly what ch.15 independently references moments later — no cross-chapter drift. Voice and canon consistent throughout (Mara's structural-engineering framing, Caleb steady, Wren blunt-with-softening-edges, first appearance of Priya's procedural-caution voice that ch.15 picks up and develops). One "something almost like fear" hedge — logged, not fixed, same tracked pattern.
- **ch.15 — no issues found — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - Word count well above floor, no changes needed.
  - No AI-tell instances (particular/the specific/the kind of all zero), no em dashes.
  - Continuity: well/cellar numbers (4/2/1) match ch.14 exactly. Priya's public-right-of-way legal reasoning is new information but a natural, consistent extension of her established procedural character; no contradiction with anything prior. No wedding-timeline references to check.
  - One "something almost like fear" hedge — logged, not fixed, same tracked pattern. Nothing to fix — verified only.
- **ch.16 — no issues found — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - Word count well above floor, no changes needed.
  - No AI-tell instances (particular/the specific/the kind of all zero), no em dashes, no rule-of-three list patterns.
  - Continuity: shift-schedule/rules mechanics are new but consistent with everything established so far (well/cellar pattern, Priya's legal caution from ch.15). No wedding-timeline references.
  - One "something close to pride" hedge, and a mild physical-contact scene-ending (Mara leaning into Caleb) consistent with the pattern flagged in the ch.41 diagnostic — both logged for the pattern record, not fixed, since no dedicated pass is authorized yet. Nothing else to fix — verified only.
- **ch.17 — no issues found — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - Word count well above floor, no changes needed.
  - No AI-tell instances (particular/the specific/the kind of all zero), no em dashes. "Actually"/"actual" appears 3 times, not excessive for chapter length.
  - Continuity, checked against full established canon (not just tell-words, per Zia's explicit instruction this pass): Caleb's "his own missing years had never involved him standing anywhere near a well" is accurate to what the characters know at this point in the story (canon resolves Denise's personal role in ch.29, which hasn't happened yet) — not a contradiction, correctly preserves the mystery. Denise's established left-hand arthritis isn't mentioned either way here — no conflict. No wedding-timeline references.
  - One "something cold settle into her chest" hedge — logged, not fixed, same tracked pattern. Nothing to fix — verified only.
- **ch.18 — issues found and FIXED — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - **Continuity/canon error (FIXED) — same class of bug as ch.12's, second occurrence:** Wren states her own age as "I'm nineteen" twice in the same exchange, directly contradicting established canon (Wren is 20) and the ch.12 fix made to the exact same fact. Both instances corrected to "twenty." Established-canon section above updated to flag Wren's age as a specifically recurring, high-risk error to double-check in every remaining chapter.
  - **AI-tell (FIXED):** removed 1 instance of "the kind of" ("it's not a small kind of wrong" → "it's a wrong that won't be small").
  - Word count comfortably above floor, no padding needed. No em dashes.
  - Voice and canon otherwise consistent: the Denise/Walt Pruitt/marriage-record reveal is new information but builds cleanly on ch.8-9's established folk-story thread without contradicting anything. Wren's solo-investigation instinct and the Mara/Caleb concerned-but-understanding reaction both consistent with their established characterizations.
- **ch.19 — no issues found — reviewed by Claude (chat session, Voxel pipeline check), 2026-09-16**
  - Word count comfortably above floor, no changes needed.
  - No AI-tell instances (particular/the specific/the kind of all zero), no em dashes.
