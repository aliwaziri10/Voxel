...
- **Cross-chapter running counts (blackout tallies, day/week countdowns) need active verification, not just internal-to-the-chapter consistency checks (added 2026-09-16).** Four real bugs of this exact type have now been found (ch.8, ch.9, ch.13, ch.15) — this is now a confirmed recurring risk category for this book, the same way "particular" was for Book 2. **Ch.15's bug specifically shows the count can drift in EITHER direction (too many days remaining, not just too few) — check the actual arithmetic each time, don't assume "close to the prior chapter's number" is automatically safe.** Any chapter that recaps a running count (blackouts, days until the eclipse, number of ring members, etc.) needs that count checked against what every earlier chapter actually established.
...
| 16-45 | NOT YET REVIEWED at the continuity/voice/canon level. Mechanically clean per the full sweep (see table above), but that is the Copy Editor role only — see charter for why this is not sufficient for "ready to publish." | — |
...
1. **Commission the full sequential continuity/voice/canon review (ch.16-45)?** This is the only thing standing between "mechanically clean" and "genuinely publish-ready" per the charter. Five genuine cross-chapter bugs found in ch.1-15 alone, none catchable by mechanical scan. **Update 2026-09-16: Zia has confirmed wanting this done in full, in detail, so chapters don't need re-checking later.**
