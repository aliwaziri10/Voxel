...
🔒 ch.14 claimed by Claude (chat session), 2026-09-22. Chapters 1-13 and
ch.15-16 are DONE. Remove this claim when ch.14 is stamped.
...
- Josiah vs Ambrose Whitlock, RESOLVED. Josiah signed the 1847 marginal note
  and the 1863 hidden letter (both predate Ambrose's 1872-1901 tenure). "A.W."
  in newer ink is Ambrose, who taught Adelaide. Fixed in ch.6 (`f9495fc`),
  ch.7 (`b98711c`), ch.9 (`0041d89`), ch.12 (`0553dba`). Ch.13 confirms this
  naming ("letters from Adelaide and Josiah Whitlock") without a fix needed.
  Check any later "Ambrose Whitlock tried/failed/hid the terms" mention.
- Drake countdown "closed" verdict is RE-OPENED: ch.9 already has the printer
  ...
  (eight weeks, council in three days for press funding), ch.16 restarts it
  again (two weeks, target date "the fifteenth", 14 days), ch.37 repeats the
  Friday print deadline. Re-check in the season/countdown pass.
- Anachronism pattern (FIXED in ch.8 and ch.12): early chapters treated the
  ...
  was deliberate" as past. Those are ch.36-45 events. Watch ch.14, 17-35.
...
2. WARDEN OF THE FIRST DEBT. Ch.3: Yusuf is the current Warden. Ch.9: Wren
   ...
   "Warden" is both a lineage and an office. Check ch.31 and ch.37.
...
7. CALENDAR. Ch.5 late summer; ch.8 reads a "year-old" notebook with Jan-Feb
   ...
   "afternoon light was golden" (no season named); ch.16 spring, apple
   blossom, orchard blooming. Ch.4's cold-case review "next month" also
   unresolved. One calendar needed.
...
10. KISS AND RUPTURE ARE EARLY. Ch.5 and ch.9 refer to the kiss interrupted by
    ...
    Beat map: first kiss ch.21, rupture ch.30. Ch.13 has Theo and Wren still
    pre-relationship/circling (Theo notices "he had softened," a moment about
    "when did you stop being an investigator" - tender but not yet a kiss),
    consistent with the beat map's later placement, not a contradiction.
11. ARCHIVE BUILDING. Ch.9: tall windows, oak table, locked cabinet, spring
    ...
    repurposed library on Main Street", new sign not yet up. Ch.13 instead
    calls it "the municipal building's back room" / "the municipal building"
    - a THIRD name/location for what should be one Archive site. Check ch.3-4
    and add to this cross-check.
12. SMALL CONTINUITY. Two Silases (Kettering ch.11, Harker ch.37). Theo's
    ...
    9-10 year gap earlier logged. Ch.13 adds: "He had been in the valley for
    eight months now" - a new duration figure to reconcile against the
    fifteen/nineteen-year SEARCH figures (those predate his arrival; "eight
    months in the valley" is presumably since he arrived and started working
    with Wren specifically - not necessarily contradictory, but log it).
    "Standing arrangement" means two things (ch.3 ledger notation; ch.4
    sealed 2005 record). The 1942 renewal gap (ch.10) is unexplained. Ch.11
    Wren's "first search... the first one" vs "paid small debts before."
    Ch.37 still has Drake "in a county holding cell" but the arrest is in
    ch.41.
13. SPACED HYPHENS " - " as em-dash substitutes (invisible to the U+2014
    checks). Counts: ch.3 ~8, ch.4 ~14, ch.6 ~3, ch.7 ~9, ch.8 3, ch.9 0,
    ch.10 6, ch.11 11, ch.12 22, ch.16 ~2, ch.13 ~1 (a cut-off dialogue
    line, "He said - ", stylistically a trail-off not a clause break -
    probably fine as-is, flagging for the sweep decision only).
14. STYLE (line-edit, not mechanical; not fixed anywhere). Staccato fragment
    ...
    ch.9, "timeline" ch.10); heavy "said" tags (ch.8, 11, 12).
15. DENISE / MARA / CALEB RELATIONSHIP, new find ch.16. Denise says at the
    ...
    else is in any other chapter (ch.13 doesn't mention this). Needs a check
    across ch.1-12, 14, 17+ (and Books 1-3) for how Denise/Mara/Caleb are
    related before touching this line.
...
- ch.13: READ. No fix. Clean chapter overall - Odette's proposal-memory scene,
  Josiah Whitlock naming consistent, no timeline breaks found. Flags: Q7
  (no season named), Q9 (care facility wording), Q11 (third Archive
  location name: "municipal building"), Q12 (new "eight months in the
  valley" duration), Q13 (one trail-off dash), Q15 (not mentioned here, no
  new evidence either way).
- ch.14: OWED. Not yet read this pass.
- ch.15: FIXED and VERIFIED by full live read 2026-09-21 (commit context
  ...
  beyond the age line.
- ch.16: READ (out of sequence — corrected by returning to ch.13-14; charter
  says never skip, this should not recur). No fix applied. Flags: Q7
  (spring/apple blossom), Q9 (care facility), Q11 (repurposed library, no
  sign yet - a FOURTH Archive location description alongside ch.13's
  "municipal building"), Q13, new Q15 (Denise's "grandson's husband" line).
  Drake's teaser email and 9am council session are internally consistent
  (2024 tracking number, Denise's 50-year renewal figure matches canon,
  nineteen-years-since-2005 Millbrook attempt matches canon). Countdown
  restarts again here (see "Decided and applied" above) - two weeks / 14
  days / target the fifteenth.
...
## Not done
- Sequential read of ch.14, then ch.17-45.
- Season/month, countdown, and Archive-building-name pass (now a THREE-name
  problem: municipal building / repurposed library on Main St / earlier
  chapters' own descriptions - see Q11). Check against Books 1-3.
