# Book 4 proofed log

## Chapter status — continuity pass
All 45 chapters: **DONE.**

| Ch | Note |
|----|------|
| 1  | clean |
| 2  | clean |
| 3  | FIXED — grandmother's age to 14 (`268ae50`) |
| 4  | clean |
| 5  | FIXED — "a Whitlock descendant" removed, ×2 (`9e3991c`) |
| 6  | clean |
| 7  | clean |
| 8  | FIXED — anachronisms |
| 9  | FIXED — "assisted living facility" (`e48fdeb`) |
| 10 | clean |
| 11 | FIXED — Reynolds to Reyes, age/decade; grandmother named Vivian (`ee44bbe`) |
| 12 | FIXED — truncated sentence, attribution, 2 anachronisms |
| 13 | clean |
| 14 | FIXED — teaching direction reversed (`b5d5d19`) |
| 15 | FIXED — full rewrite verified |
| 16 | clean |
| 17 | clean — first kiss |
| 18 | FIXED — day-count contradiction |
| 19 | clean — anchor chapter |
| 20 | FIXED — timeline reset (`da6ebe0`) |
| 21 | FIXED — custody (`beb46da`), Dev's evidence (`11cd9f0`) |
| 22 | clean — "assisted living facility" (`990a104`) |
| 23 | FIXED — Harriet's age |
| 24 | FIXED — grandmother's age to 14 (`e82970a`) |
| 25 | FIXED — age, custody, em dash (`f9e01f5`) — the 2 literal em dashes here are the one deliberate kept exception |
| 26 | FIXED — solstice/custody, kiss ref (`c4dbf27`) |
| 27 | clean |
| 28 | clean |
| 29 | clean |
| 30 | clean |
| 31 | FIXED — rupture-scene insert (`0963500`) |
| 32 | clean |
| 33 | clean |
| 34 | FIXED — custody (`866eb6b`) |
| 35 | clean |
| 36 | clean |
| 37 | clean |
| 38 | clean |
| 39 | FIXED — custody claims removed |
| 40 | FIXED — custody claims removed |
| 41 | FIXED — age (`29be7e8`); canonical arrest scene |
| 42 | clean |
| 43 | clean |
| 44 | clean |
| 45 | clean |

## Mechanical/lexical/tell-density pass, ch.27-45
Scope: literal em-dash count, repeated words/phrases, staged/parallel
constructions, general tell-density.

| Ch | Status | Note |
|----|--------|------|
| 27 | CLEAN | Zero literal em dashes (spaced-hyphen style throughout). One repeated parallel construction ("Not because X. Not because Y. Because Z.") used twice as a deliberate mirrored-vow device. |
| 28 | CLEAN | Zero literal em dashes. "Nine days" repeated as a deliberate countdown refrain. "There it is" repeated 3x in the Priya/Wren scene, a deliberate rhythmic beat. |

**Remaining: ch.29-45, not yet started.**

## Decisions locked (facts, do not re-derive)
1. Josiah Whitlock (recorder, 1847) is not Ambrose Whitlock (Warden,
   1872-1901) — father and son, distinct people.
2. Countdown chain ch.16-21 = one deadline (Friday-the-11th). Ch.31-37 =
   a second, later clock. Two crises, not a broken timeline.
3. Ch.41 (home, 6:45am) is the one canonical arrest; ch.21/25/26/34/
   39/40 all match it.
4. Kiss order: ch.17 (first) → ch.21 (interrupted) → ch.26 (deepest).
5. Grandmother's death age = 14, standardized.
6. Harriet Marsh's home = "assisted living facility" / Willow Creek,
   standardized.
7. Silas Kettering and Silas Harker are distinct characters, already
   differentiated on the page (ch.5/11 vs. ch.37/38).
8. Martha Whitlock = Ambrose's daughter, confirmed book-wide (ch.22).
9. Denise's "granddaughter's husband" line (ch.16) is correct as is —
   refers to Caleb.
10. Drake's em dash (ch.25, ×2): the one deliberate exception to the
    book-wide em-dash ban. Never add or "correct" any other em dash.
11. Ch.5's "-A.W." journal signature is a deliberate misdirection
    (Ambrose vs. Adelaide Whitlock, same initials).
12. Ambrose Whitlock taught Adelaide the bargain, not the reverse
    (ch.14 fixed to match).
13. Elena Castellano (Wren's mother) is a retired baker. Wren's
    grandmother — previously also named Elena in ch.11 — is now
    Vivian Castellano (`ee44bbe`), matching the same collision fix
    already used for the great-great-grandmother (Clara).
14. Publication timeline: original target = summer solstice
    (superseded); actual target = spring, "Opening Spring" banner.
15. The Amity Falls Archive = the renovated old Main Street library
    (flood of '98, rebuilt on original foundation stones). Earlier
    "barn/shed/room above the store" mentions are the temporary
    workroom used before the real building was ready.
16. Drake's Millbrook Finder is Silas Kettering (ch.11); ch.5 no
    longer names or misnames him as a Whitlock descendant.
17. "The house on the ridge" (ch.22) is Theo and Wren's shared home,
    which Dev has also come to treat as home; Priya has her own,
    separate home.
18. Spaced hyphens (" - ") are the book-wide, deliberate substitute
    for the charter's banned em dash — confirmed by direct count (2
    literal em dashes in all 45 chapters combined, both the logged
    Drake exception). Never convert a spaced hyphen to an em dash.
19. "Harlan" is both a law firm name (ch.23) and, separately, an
    unrelated place name (ch.27) — not the same entity.
20. Holloway & Finch (dissolved 2003) and Meridian Consulting (active)
    are two sequential shell companies in Drake's long-running scheme,
    not duplicate names for one company.
21. Theo Marsh is 26 in both ch.1 and ch.15, self-consistent (16 + 10
    years). No birth year is stated in the manuscript.
22. Ch.2's county-records-audit threat (Ordinance 44-12, 14 days) is
    the inciting incident for the book's publication race; ch.16's
    publication plan is its answer.
23. Wren's grandmother (1985 Miller-girl search, ch.11) is Vivian
    Castellano, distinct from Wren's mother Elena Castellano.
24. "Castellan" vs. "Castellano" is a deliberate near-miss, not an
    error.
25. Book-wide lineage roster (as far as the text supports): Finders —
    Wren Castellano, her grandmother Vivian, her mother Elena, Dev
    Nair. Callers — Priya Nair. Menders — Mara Voss. Wardens —
    historically Whitlock line, now consensus-based. Weathers — three
    members, only Silas Harker named. Wicks — thinnest lineage, no
    members named on the page.
26. The 14-day "Notice of Records Audit" clock (ch.2) is answered by
    ch.16's fifteenth-of-the-month publication plan — no separate
    ending needed.

## Genuinely open (low priority, self-contained, flagged for tracking)
- Archive building's earlier physical descriptions (barn, shed, room
  above the store) vs. the final library-on-the-square description are
  reconciled per Decision #15 above; no further action needed unless a
  new chapter edit reintroduces a conflict.
- 1942 renewal gap (ch.10): mentioned in passing, not yet explored.
- Fading-memory thread (ch.11): Wren's own search debt is eroding
  rather than staying fixed, unlike every other documented debt.
  Ch.27-45 don't pay this off explicitly — worth tracking if a later
  revision touches these chapters.
- 1893 ledger vs. oral-tradition Mender-toll numbers (ch.12): the
  chapter explicitly resolves this as "both true in different
  registers" — intentional, not an error, do not "fix" if seen again.
- 1984 renewal gap (ch.35, ch.43): a separate numeric gap from the
  1887-1891 folio gap and the 1942 gap above — three distinct "missing
  renewal" threads. Worth a check that none are the same event given
  different years by mistake, but not flagged as a live contradiction.
- `amity-falls-book-4_full_manuscript.md` (the concatenated file) is
  stale relative to individually-fixed chapters (still shows pre-rename
  "Elena Castellano" for the ch.3 folio signer, and an outdated Theo-age
  line for ch.15). Do not trust it for naming- or number-sensitive
  checks — use the individual chapter files. Fine for fast greps to
  locate a chapter to check.

## Next step
Mechanical/lexical/tell-density pass, ch.29-45 (ch.27-28 already done,
see table above). After that: Books 1-3 cross-check.

_Full session-by-session history, including every correction and
superseded note, is preserved in git history for this file if the
reasoning behind any line above is ever needed again._
