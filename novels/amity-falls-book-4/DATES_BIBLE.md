# Book 4 — Dates & Facts Bible

Living reference, updated as the sequential read progresses. Every entry
notes which chapter(s) it's confirmed in. If a later chapter contradicts an
anchor here, do NOT silently overwrite this file — flag it, then decide
whether the anchor or the new chapter is wrong, fix the wrong one in its own
chapter file, and update this bible to match. This file is the single
source of truth other sessions should check before touching any chapter.

## ANCHORED (settled — treat as correct, fix any chapter that contradicts)

- **Missing folio gap: 1887–1891.** 1887–89 suspended, 1890–91 resumed.
  Last folio present before gap: 1886. First folio back: 1892.
  (ch.3, fixed commit `207db70`)
- **Warden succession:** J. Whitlock (role active 1847, still active 1863,
  tried and failed to end the bargain both times, hid the terms rather than
  destroy them) → Ambrose Whitlock (Warden 1872–1901, tried again 1889,
  hid Adelaide's letter) → Sarah Voss/Mara's family (1901–1947, Menders)
  → Weathers line → Caller line → vacant after prior Warden died 1987 →
  Yusuf (Warden through the ring-ritual era) → **role passes to Wren**
  by custody of the ledger, not blood (ch.9 — "whoever holds the ledger
  holds the duty"). Two separate Whitlocks (Josiah, ~1847–63; Ambrose,
...
- **Present-day timeframe:** ring ritual happened "a year ago"; publication
  ...
  from the ch.9 scene.**
- **Bargain renewal record (modern/administrative sense):** "everyone who
  renewed the bargain since 1972" — Drake's partial record's start point.
...
- **chapter_15 Theo age 36:** confirmed error (see anchors above), not
